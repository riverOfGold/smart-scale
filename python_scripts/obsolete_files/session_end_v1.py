...
database1 = "/home/pi/database_files/smart_scale.db"
database2 = "/home/pi/database_files/trans_data.db"
...
class SessionEndScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.units2_label = tk.Label(self.summary_frame, borderwidth=0,
                                 ...
                                 font=(default_font, large_size, default_style))
        self.next_email_btn = tk.Button(self.summary_frame, borderwidth=0,
                                        text='\u25BC', bg="white", fg="black",
                                        font=(default_font, 9, default_style),
                                        width=1,
                                        command = lambda: self.next_email(self.actual_email_label['text'], controller))
        self.prev_email_btn = tk.Button(self.summary_frame, borderwidth=0,
                                        text='\u25B2', bg="white", fg="black",
                                        font=(default_font, 9, default_style),
                                        width=1,
                                        command = lambda: self.prev_email(self.actual_email_label['text'], controller))
        self.reset_btn = tk.Button(self.btn_frame, bd=2,
                                   text="BACK TO START", fg="black",
                                   font=(default_font, small_size,
                                         default_style), bg="white",
                                   command = lambda: self.reset_scale(controller),
                                   width = 15)
        self.off_btn = tk.Button(self.btn_frame, bd=2,
                                 text="POWER OFF", fg="black",
                                 font=(default_font, small_size,
                                       default_style),bg="white",
                                 command=lambda: self.power_off(),
                                 width = 15)
        self.back_btn = tk.Button(self.btn_frame, bd=2,
                                  text="BACK TO SESSION", fg="black",
                                  font=(default_font, small_size,
                                        default_style), bg="white",
                                  command=lambda: controller.show_frame(SR.SessionStartScreen),
                                  width = 15)
...
#        self.activate_control(self.reset_btn)
    ...
    def prev_email(self, current_email, controller):
        ...
        # finally, update the settings_screen_v1 frame in case the user navigates back to there
        # at some point
        controller.frames[SG.SettingsScreen].email_entry.delete(0, tk.END)
        controller.frames[SG.SettingsScreen].email_entry.insert(tk.END, new_email)
